...
> **Version:** 1.9.13 (Last Synced: 2026-07-30)
...
| D33 | ADR-040 เป็น Source of Truth สำหรับ OCR sidecar contract (engine selection, /normalize removal) — ADR-035 OCR sidecar section ถูก amend อย่างเป็นทางการ (Amended by note ใน header)                                                                                                                                                                                  | ADR-040 D1/D2      |
...
| D35 | Audit log ต้องสะท้อนความจริง: `auto-fallback`/`auto` (ไม่ใช่ `pymupdf`) เมื่อส่ง `engine='auto'` ไป sidecar — ห้ามเขียน audit ที่ไม่ตรง engine จริง                                                                                                                                                                                                                  | ADR-040            |
| D36 | Sidecar `_process_pdf_doc`: `auto` เป็น known engine — ลอง PyMuPDF text layer ก่อน → fallback ไป np-dms-ocr โดยตรง (ไม่ใช่ "Unknown engine"); ทุก engine path นำไปสู่ np-dms-ocr (ลบ code duplication)                                                                                                                                                               | ADR-040 D1         |
| D37 | `docs/AI-step.md` deprecated — ใช้ `specs/02-architecture/02-05-ai-document-ingestion-flow.md` เป็น canonical AI ingestion flow walkthrough (ควรลบหรือทำเป็น redirect ในอนาคต)                                                                                                                                                                                       | Session 2026-07-30 |
| D38 | ADR-040 Phase 2 complete — X-API-Key auth ถูกลบทั้งหมดจาก sidecar + backend; ใช้ Docker-internal network isolation แทน (ADR-041 consolidation complete)                                                                                                                                                                                                              | ADR-040 D6 Phase 2 |
| D39 | `OCR_SIDECAR_API_KEY` env var ไม่มีอยู่แล้ว — ห้ามเพิ่มกลับมา; ลบจาก docker-compose, .env.template, .env.example, MIGRATION-PLAN, backend/.env.example                                                                                                                                                                                                               | ADR-040 Phase 2    |
| D40 | Sidecar endpoints (`/ocr`, `/ocr-upload`, `/embed`, `/rerank`) ไม่ต้องมี auth header — พึ่ง network isolation เท่านั้น; curl testing guide อัปเดตแล้ว                                                                                                                                                                                                                | ADR-040 D6         |
| D41 | `test_api_key_validation.py` ถูกลบ — ห้ามสร้างใหม่ (feature ไม่มีแล้ว); test files อื่นๆ ลบ `X-API-Key` headers + `OCR_SIDECAR_API_KEY` env setup ออกหมด                                                                                                                                                                                                             | ADR-040 Phase 2    |
...
### Feature 241: OCR Text Persistence & Sandbox Project (Session 2026-07-30) ✅ CODE COMPLETE

- [x] **Phase 1-6:** 23/23 tasks ทำเสร็จ (T001-T023)
- [x] **DB Delta Applied:** ocr_text + is_sandbox columns + SANDBOX project seed (id=7)
- [ ] **Verify:** `tsc --noEmit` (backend) — ยืนยันไม่มี type error หลังแก้ AiBatchProcessor + AiService
- [ ] **Verify:** `pnpm test` (backend) — ยืนยัน rag-prepare tests ผ่าน (T006, T007)
- [ ] **Verify:** `pnpm build` (frontend) — ยืนยัน SandboxTabs.tsx compile ได้
- [ ] **Manual Test:** quickstart.md ข้อ 2 — OCR text persistence เมื่อ embedding fail
- [ ] **Manual Test:** quickstart.md ข้อ 3 — Full Pipeline ผ่าน Sandbox Project + Clear Data
- [ ] **Manual Test:** quickstart.md ข้อ 4 — regular user ไม่เห็น SANDBOX project
- [ ] **Manual Test:** quickstart.md ข้อ 5 — Production Pipeline Sandbox เดิมไม่ commit DB
...
- [ ] **ลบ `docs/AI-step.md`** หรือทำเป็น redirect → 02-05 (ต้องได้รับอนุมัติ — destructive)
- [ ] **Build + Deploy** — rebuild backend + sidecar image เพื่อให้การแก้ code smell + Phase 2 มีผลใน production
...
### Gitea Runner Port Fix (Session 2026-07-20)
...
- [ ] Deploy บน ASUSTOR: แก้ `.env` จริง + ลบ `.runner` cache + restart container
...
- [ ] Deploy: รัน `copy-env.sh` + `dockerup.sh` บน server
...
- [ ] **Rebuild:** `cd /opt/np-dms/04-ai/ocr-sidecar && sudo docker compose --env-file ../../.env up -d --build --no-cache`
- [ ] **Quickstart validation:** ทดสอบ Test 1-4 ตาม `specs/100-Infrastructures/142-ocr-prompt-cache-invalidation/quickstart.md`
