// File: backend/src/modules/migration/services/gemini-review.adapter.ts
// Change Log:
// - 2026-09-11: Initial creation — Gemini AI Reviewer Adapter skeleton (Phase D, FR-008, D2)
//   ใช้ Google Gemini API สำหรับ Layer 3 review (External AI — ต้องมี Admin + ALLOW_EXTERNAL_AI_REVIEW=true)
...
/**
 * GeminiReviewAdapter — Adapter สำหรับ Google Gemini API (FR-008, D2)
 *
 * ใช้ Gemini Pro สำหรับ Layer 3 review — ส่งเฉพาะข้อมูลจำเป็น
 ...
 */
@Injectable()
export class GeminiReviewAdapter implements AiReviewerAdapter {
  private readonly logger = new Logger(GeminiReviewAdapter.name);
  readonly provider: AiReviewerProvider = 'GEMINI';
  ...
  isAvailable(): Promise<boolean> {
    if (!this.apiKey) {
      this.logger.debug('Gemini API key ไม่ได้ตั้งค่า — ไม่พร้อมใช้งาน');
  ...
  async review(input: AiReviewInput): Promise<ReviewFinding[]> {
    if (!this.apiKey) {
      this.logger.warn('Gemini API key ไม่ได้ตั้งค่า — Fail-Open');
      return [];
    }
    ...
    for (const row of input.rows) {
      ...
      try {
        ...
        this.logger.debug(
          `Gemini review ล้มเหลวสำหรับแถว ${row.rowIndex}: ${detail}`
        );
      }
    }
  ...
  /**
   * ดึง text จาก Gemini API response
   */
