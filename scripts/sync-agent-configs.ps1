...
$devinDir = Join-Path $rootDir ".devin"

Write-Host "🚀 Syncing .agents to .devin (v1.9.0)..." -ForegroundColor Cyan

# 1. Sync Rules (with YAML header)
Write-Host "📂 Syncing Rules..." -ForegroundColor Yellow
$rulesSrc = Join-Path $agentsDir "rules"
$rulesDest = Join-Path $devinDir "rules"
...
$workflowsDest = Join-Path $devinDir "workflows"
...
$skillsDest = Join-Path $devinDir "skills"
