...
**Version 1.8.9**
**NAP-DMS / LCBP3** — Laem Chabang Port Phase 3 Document Management System
2026-04-22
...
4. [Security](#4-security) — **HIGH**
   ...
   - 4.2 [Two-Phase File Upload + ClamAV (ADR-016)](#42-two-phase-file-upload-clamav-adr-016-)
   ...
   - 4.6 [Validate All Input with DTOs and Pipes](#46-validate-all-input-with-dtos-and-pipes)
...
### 4.2 Two-Phase File Upload + ClamAV (ADR-016)

**Impact: CRITICAL** — Upload → Temp → ClamAV scan → Commit → Permanent. Whitelist + 50MB cap. StorageService only.
...
### 4.6 Validate All Input with DTOs and Pipes
...
### 7.2 Hybrid Identifier Strategy (ADR-019)

**Impact: CRITICAL** — Use INT PK internally + UUID for public API per project ADR-019
...
### 7.5 Use Transactions for Multi-Step Operations
...
### 11.2 Workflow Engine + Document Numbering + Workflow Context (ADR-001 / 002 / 021)

**Impact: CRITICAL** — DSL-based state machine; double-lock numbering; integrated workflow context exposed to clients.
