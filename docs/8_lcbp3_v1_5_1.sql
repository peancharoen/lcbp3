...
CREATE TABLE shop_drawing_revisions (
    ...
    UNIQUE KEY ux_sd_rev_drawing_revision (
        shop_drawing_id,
        revision_number
    )
...
ALTER TABLE correspondence_revisions
ADD COLUMN schema_version INT DEFAULT 1 COMMENT 'เวอร์ชันของ Schema ที่ใช้กับ details' AFTER details;
...
ALTER TABLE rfa_revisions
ADD COLUMN schema_version INT DEFAULT 1 COMMENT 'Version ของ JSON Schema' AFTER details;
...
CREATE INDEX idx_workflow_active ON workflow_definitions (
    workflow_code,
    is_active,
    version
);
...
CREATE INDEX idx_document_number_formats_project_type ON document_number_formats (
    project_id,
    correspondence_type_id
);
...
CREATE INDEX idx_audit_logs_reporting ON audit_logs (
    created_at,
    entity_type,
    ACTION
);
-- Composite index for search_indices for entity-based queries
CREATE INDEX idx_search_entities ON search_indices (
    entity_type,
    entity_id,
    indexed_at
);
...
SELECT -- 1. Workflow Instance Info
    ...
    JSON_UNQUOTE(
        JSON_EXTRACT(wi.context, '$.ownerId')
    ) AS owner_id,
...
SELECT al.audit_id, al.user_id, u.username, u.email, u.first_name, u.last_name, al.action, al.entity_type, al.entity_id, al.details_json, al.ip_address, al.user_agent, al.created_at
FROM audit_logs al
...
CREATE INDEX idx_correspondences_type_project ON correspondences (
    correspondence_type_id,
    project_id
);

CREATE INDEX idx_corr_revisions_current_status ON correspondence_revisions (
    is_current,
...
CREATE INDEX idx_rfa_revisions_current_status ON rfa_revisions (
    is_current,
    rfa_status_code_id
);
...
CREATE INDEX idx_correspondences_project_type ON correspondences (
    project_id,
    correspondence_type_id
);

CREATE INDEX idx_corr_revisions_status_current ON correspondence_revisions (
    correspondence_status_id,
    is_current
);
