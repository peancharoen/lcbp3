...
-- Update: first revise fron v1.3.0
-- ==========================================================
...
-- ==========================================================
-- ตาม Requirements 6.6 ที่ระบุว่าต้องมีการสำรองข้อมูล
...
-- ตารางสำหรับจัดการดัชนีการค้นหา
...
INSERT INTO roles (role_code, role_name, description, is_system) VALUES
  ...
  ('DOC_CONTROL','Document Control',    'Manages all documents within their organization',  1),
  ('EDITOR',     'Editor',              'Can create/edit specific documents',               1),
...
INSERT INTO permissions (permission_code, description, module, scope_level) VALUES
  -- System & Admin Panel (Req 4.5, 4.6, 6.1)
  ('system.superadmin',       'Full access to everything (Bypasses all checks)', 'System', 'GLOBAL'),
  ('system.admin_panel',      'Access Admin Panel UI', 'System', 'ORG'),
  ('system.view_audit_logs',  'View audit logs for own organization', 'System', 'ORG'),
  ('system.view_reports',     'View reports page', 'System', 'ORG'),

  -- Admin Panel Actions (Req 4.3, 4.5, 4.6, 3.10.3)
  ('admin.manage_users',      'Create/Edit/Delete users in own organization', 'Admin', 'ORG'),
  ('admin.manage_roles',      'Create/Edit/Delete custom roles and assign permissions', 'Admin', 'ORG'),
  ('admin.manage_master_data','Manage master data (tags, types, categories)', 'Admin', 'ORG'),
  ('admin.manage_numbering',  'Manage document numbering formats', 'Admin', 'ORG'),
  ('admin.manage_org_all',    '[Superadmin] Manage all organizations', 'Admin', 'GLOBAL'),
  ('admin.manage_users_all',  '[Superadmin] Manage all users in all organizations', 'Admin', 'GLOBAL'),

  -- Projects & Contracts (Req 3.1)
  ('project.view',            'View assigned projects', 'Project', 'PROJECT'),
  ('project.manage',          '[Admin] Create/Edit projects', 'Project', 'GLOBAL'),
  ('project.manage_parties',  '[Admin] Assign organizations to projects', 'Project', 'PROJECT'),

  -- Correspondences (Req 3.2)
  ('corr.view',               'View documents sent to/from own organization', 'Correspondence', 'PROJECT'),
  ('corr.view_all',           '[Admin/Owner] View all documents in a project', 'Correspondence', 'PROJECT'),
  ('corr.create',             'Create document drafts', 'Correspondence', 'PROJECT'),
  ('corr.submit',             'Submit a draft document', 'Correspondence', 'PROJECT'),
  ('corr.manage_submitted',   '[Admin] Edit/Cancel/Delete submitted documents', 'Correspondence', 'PROJECT'),
  ('corr.manage_attachments', 'Add/Remove/Manage attachments', 'Correspondence', 'PROJECT'),

  -- RFA (Req 3.5, 5.6)
  ('rfa.view',                'View RFAs sent to/from own organization', 'RFA', 'PROJECT'),
  ('rfa.view_all',            '[Admin/Owner] View all RFAs in a project', 'RFA', 'PROJECT'),
  ('rfa.create',              'Create new RFA (all types: DWG, DOC, MAT, MES)', 'RFA', 'PROJECT'),
  ('rfa.submit',              'Submit RFA draft', 'RFA', 'PROJECT'),
  ('rfa.respond',             'Respond to an RFA workflow step (Approve/Reject)', 'RFA', 'PROJECT'),
  ('rfa.override_workflow',   '[Admin] Force bypass/return workflow steps', 'RFA', 'PROJECT'),
  ('rfa.delete',              'Delete RFA drafts', 'RFA', 'PROJECT'),

  -- Drawings (Req 3.3, 3.4)
  ('drawing.contract.view',   'View Contract Drawings', 'Drawing', 'PROJECT'),
  ('drawing.contract.manage', 'Upload/Edit/Delete Contract Drawings', 'Drawing', 'PROJECT'),
  ('drawing.shop.view',       'View Shop Drawings', 'Drawing', 'PROJECT'),
  ('drawing.shop.manage',     'Upload/Edit/Delete Shop Drawing Masters & Revisions', 'Drawing', 'PROJECT'),

  -- Transmittals (Req 3.6)
  ('transmittal.view',        'View Transmittals', 'Transmittal', 'PROJECT'),
  ('transmittal.create',      'Create new Transmittals', 'Transmittal', 'PROJECT'),

  -- Circulations (Req 3.7)
  ('circulation.view',        'View own organizations circulations', 'Circulation', 'ORG'),
  ('circulation.create',      'Create new internal circulation sheet', 'Circulation', 'ORG'),
  ('circulation.respond',     'Action on an assigned task (Comment, Close)', 'Circulation', 'ORG'),
  ('circulation.close',       'Mark a circulation as completed', 'Circulation', 'ORG'),
  ('circulation.manage_templates', 'Create/Edit circulation templates for own org', 'Circulation', 'ORG')
ON DUPLICATE KEY UPDATE
  description=VALUES(description),
...
CREATE TABLE notifications (
  ...
  message TEXT,
  notification_type ENUM('EMAIL', 'LINE', 'SYSTEM') NOT NULL,
...
CREATE TABLE correspondences (
  ...
  -- recipient_id               INT NULL COMMENT 'องกรณ์ที่เป็นปู้รับเอกสาร', -- organizations.id -> correspondence_recipients
...
CREATE INDEX idx_cor_deleted_at ON correspondences(deleted_at);
...
CREATE TABLE circulations (
  ...
  CONSTRAINT fk_cir_status FOREIGN KEY (circulation_status_code) REFERENCES circulation_status_codes(code),
...
CREATE INDEX idx_rfa_deleted_at ON rfas(deleted_at);
...
-- (ข้อควรทราบ:
-- 1. Constraint 'fk_rr_correspondence' (FOREIGN KEY) บน 'correspondence_id'
--    ยังคงอยู่ และทำหน้าที่เป็น M:1 อย่างถูกต้อง
-- 2. Constraint 'fk_rr_rfa' (FOREIGN KEY) บน 'rfa_id' ยังคงอยู่
-- 3. UNIQUE KEY 'uq_rr_rev_number' (rfa_id, revision_number) ยังคงอยู่
-- 4. UNIQUE KEY 'uq_rr_current' (rfa_id, is_current) ยังคงอยู่
-- )
...
CREATE INDEX idx_rr_created_at     ON rfa_revisions(created_at);

-- 2.9.1 correspondence_revisions Table: ตารางสำหรับเก็บประวัติการแก้ไขเอกสาร (Revision History)
-- (ข้อควรทราบ:
-- 1. Constraint 'fk_cr_correspondence' (FOREIGN KEY) บน 'correspondence_id' ยังคงอยู่
--    ซึ่งตอนนี้จะทำหน้าที่เป็น M:1 (Many-to-One) อย่างถูกต้อง
-- 2. UNIQUE KEY 'uq_master_revision_number' (correspondence_id, revision_number)
--    ยังคงอยู่ เพื่อป้องกัน Rev 1, Rev 1 ซ้ำกันใน Master เดียว
-- 3. UNIQUE KEY 'uq_master_current' (correspondence_id, is_current)
--    ยังคงอยู่ เพื่อบังคับให้มี is_current=TRUE ได้เพียงอันเดียวต่อ Master
-- )
...
CREATE INDEX idx_cr_created_at ON correspondence_revisions(created_at);
...
-- ==========================================================
-- ตารางกำหนด "รูปแบบ" เลขที่เอกสาร (Configuration)
-- ==========================================================
CREATE TABLE document_number_formats (
  id INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
  project_id INT NOT NULL COMMENT 'FK -> projects.id (กำหนดรูปแบบตามโครงการ)',
  correspondence_type_id INT NOT NULL COMMENT 'FK -> correspondence_types.id (กำหนดรูปแบบตามประเภทเอกสาร)',

  -- Template สำหรับการสร้างเลขที่
  -- เช่น "{ORG_CODE}-{TYPE_CODE}-{YEAR_SHORT}-{SEQ:4}"
  -- {PROJECT_CODE}, {ORG_CODE}, {TYPE_CODE}, {YEAR_FULL}, {YEAR_SHORT}, {MONTH}, {DAY}, {SEQ:X}
  format_template VARCHAR(255) NOT NULL COMMENT 'รูปแบบ Template ของเลขที่เอกสาร',

  description TEXT NULL COMMENT 'คำอธิบายรูปแบบนี้',

  UNIQUE KEY uk_project_type (project_id, correspondence_type_id),

  CONSTRAINT fk_dnf_project FOREIGN KEY (project_id) REFERENCES projects(id) ON DELETE CASCADE,
  CONSTRAINT fk_dnf_corr_type FOREIGN KEY (correspondence_type_id) REFERENCES correspondence_types(id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='ตารางเก็บ "รูปแบบ" การสร้างเลขที่เอกสาร';

-- ==========================================================
-- ตาราง "ตัวนับ" เลข Running (Counter)
-- ==========================================================
-- ตารางนี้จะเก็บเลขที่ล่าสุด โดยใช้ Key ที่คุณระบุมา
CREATE TABLE document_number_counters (
  -- Key หลักตามข้อกำหนด: Project + Org + Type
  project_id INT NOT NULL COMMENT 'FK -> projects.id',
  originator_organization_id INT NOT NULL COMMENT 'FK -> organizations.id (ตามข้อกำหนด: องค์กรผู้ส่ง)',
  correspondence_type_id INT NOT NULL COMMENT 'FK -> correspondence_types.id (ตามข้อกำหนด: ประเภทเอกสาร)',

  -- ตัวนับมักจะ Reset รายปี
  current_year INT NOT NULL COMMENT 'ปี ค.ศ. ปัจจุบันของตัวนับนี้',

  -- ค่าตัวนับล่าสุด
  last_number INT NOT NULL DEFAULT 0 COMMENT 'เลขที่ล่าสุดที่ใช้ไปแล้ว',

  -- PK ที่สมบูรณ์
  PRIMARY KEY (project_id, originator_organization_id, correspondence_type_id, current_year),

  CONSTRAINT fk_dnc_project FOREIGN KEY (project_id) REFERENCES projects(id) ON DELETE CASCADE,
  CONSTRAINT fk_dnc_org FOREIGN KEY (originator_organization_id) REFERENCES organizations(id) ON DELETE CASCADE,
  CONSTRAINT fk_dnc_corr_type FOREIGN KEY (correspondence_type_id) REFERENCES correspondence_types(id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='ตาราง "ตัวนับ" เลขที่เอกสาร (Running Number)';
-- เพิ่ม Index สำหรับการค้นหาที่ใช้บ่อย
CREATE INDEX idx_dnc_project_org ON document_number_counters(project_id, originator_organization_id);
CREATE INDEX idx_dnc_type_year ON document_number_counters(correspondence_type_id, current_year);
-- ==========================================================
-- Level 1: ตารางที่มีความสัมพันธ์มากที่สุด (Junction Tables & Detail Tables)
-- ==========================================================
...
CREATE TABLE shop_drawing_revision_attachments (
  ...
  is_main_document BOOLEAN NOT NULL DEFAULT FALSE COMMENT 'ระบุว่าเป็นไฟล์หลักหรือไม่',
...
CREATE TABLE contract_drawing_subcat_cat_maps (
  ...
  CONSTRAINT fk_map_subcat FOREIGN KEY (project_id, sub_cat_id) REFERENCES contract_drawing_subcats(project_id, sub_cat_id) ON UPDATE CASCADE ON DELETE CASCADE,
  CONSTRAINT fk_map_cat    FOREIGN KEY (project_id, cat_id)     REFERENCES contract_drawing_cats(project_id, cat_id) ON UPDATE CASCADE ON DELETE CASCADE
...
CREATE TABLE shop_drawing_revisions (
  ...
  revision_number INT NOT NULL COMMENT 'หมายเลข Revision (สำหรับเรียงลำดับ)',
  revision_label  VARCHAR(10) NULL COMMENT 'Revision ที่แสดง (เช่น A, B, หรือ 1, 2)',
...
CREATE TABLE transmittal_items (
  ...
  item_correspondence_id  INT NOT NULL COMMENT 'ID ของเอกสารที่แนบไป (FK -> correspondences.id)',
  ...
  CONSTRAINT fk_ti_item_correspondence FOREIGN KEY (item_correspondence_id) REFERENCES correspondences(id) ON DELETE CASCADE
...
CREATE TABLE circulation_status_transitions(
  ...
  CONSTRAINT fk_cstt_from   FOREIGN KEY (from_status_code)  REFERENCES circulation_status_codes(code),
  CONSTRAINT fk_cstt_to     FOREIGN KEY (to_status_code)    REFERENCES circulation_status_codes(code)
...
CREATE TABLE circulation_assignees (
  ...
  completed_at        DATE NULL COMMENT 'เวลาที่ดำเนินการเสร็จสิ้น',
...
-- เพิ่ม Index สำหรับคอลัมน์ due_date
CREATE INDEX idx_cr_due_date ON correspondence_revisions(due_date);
...
-- ==========================================================
-- 3. Seed Role-Permissions Mapping (จับคู่สิทธิ์เริ่มต้น)
-- ==========================================================
TRUNCATE TABLE role_permissions;

-- ---------------------------------
-- 3.1 SUPER_ADMIN (สิทธิ์ทั้งหมด)
-- ---------------------------------
...
JOIN permissions p ON p.permission_code IN (
  -- System
  'system.admin_panel', 'system.view_audit_logs', 'system.view_reports',
  -- Admin Actions
  'admin.manage_users', 'admin.manage_roles', 'admin.manage_master_data', 'admin.manage_numbering',
  -- Project
  'project.view', 'project.manage', 'project.manage_parties',
  -- Correspondence
  'corr.view', 'corr.view_all', 'corr.create', 'corr.submit', 'corr.manage_submitted', 'corr.manage_attachments',
  -- RFA
  'rfa.view', 'rfa.view_all', 'rfa.create', 'rfa.submit', 'rfa.respond', 'rfa.override_workflow', 'rfa.delete',
  -- Drawing
  'drawing.contract.view', 'drawing.contract.manage', 'drawing.shop.view', 'drawing.shop.manage',
  -- Transmittal
  'transmittal.view', 'transmittal.create',
  -- Circulation
  'circulation.view', 'circulation.create', 'circulation.respond', 'circulation.close', 'circulation.manage_templates'
)
WHERE r.role_code = 'ADMIN';

-- ---------------------------------
-- 3.3 DOC_CONTROL (สิทธิ์จัดการเอกสาร)
-- ---------------------------------
INSERT IGNORE INTO role_permissions (role_id, permission_id)
SELECT r.role_id, p.permission_id
FROM roles r
JOIN permissions p ON p.permission_code IN (
  ...
  'project.view',
  -- Correspondence
  'corr.view', 'corr.create', 'corr.submit', 'corr.manage_attachments',
  -- RFA
  'rfa.view', 'rfa.create', 'rfa.submit', 'rfa.respond', 'rfa.delete',
  -- Drawing
  'drawing.contract.view', 'drawing.contract.manage', 'drawing.shop.view', 'drawing.shop.manage',
  -- Transmittal
  'transmittal.view', 'transmittal.create',
...
WHERE r.role_code = 'DOC_CONTROL';

-- ---------------------------------
-- 3.4 EDITOR (สิทธิ์สร้าง/แก้ไข)
-- ---------------------------------
INSERT IGNORE INTO role_permissions (role_id, permission_id)
SELECT r.role_id, p.permission_id
FROM roles r
JOIN permissions p ON p.permission_code IN (
  ...
  'transmittal.view', 'transmittal.create',
  -- Circulation
  'circulation.view', 'circulation.create', 'circulation.respond', 'circulation.close'
  -- Editor ไม่สามารถจัดการ Drawing หรือ Template ได้
)
WHERE r.role_code = 'EDITOR';

-- ---------------------------------
-- 3.5 VIEWER (สิทธิ์อ่านอย่างเดียว)
-- ---------------------------------
INSERT IGNORE INTO role_permissions (role_id, permission_id)
SELECT r.role_id, p.permission_id
FROM roles r
JOIN permissions p ON p.permission_code IN (
  ...
  'rfa.view',
  -- Drawing
  'drawing.contract.view', 'drawing.shop.view',
  -- Transmittal
  'transmittal.view',
  -- Circulation
  'circulation.view', 'circulation.respond' -- (สามารถ Respond งานที่ถูกส่งมาหาได้)
)
...
-- Sample seed for user_project_roles (assign superadmin)
...
-- Assign editor01 as VIEWER in project LCBP3C2
INSERT IGNORE INTO user_project_roles (user_id, project_id, role_id)
SELECT u.user_id, p.id, r.role_id
FROM users u
...
-- ==========================================================
-- ==========================================================
-- สร้าง Stored Procedure ใหม่ (สำหรับ Document Numbering)
-- ==========================================================
