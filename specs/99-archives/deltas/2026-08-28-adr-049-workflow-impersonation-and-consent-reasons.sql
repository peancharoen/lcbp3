...
-- NOTE (2026-09-07): rfa_consent_reasons.code VARCHAR(20) สั้นเกินไป —
--   'AGREED_WITH_CONDITIONS' (22) และ 'FORWARDED_TO_DESIGNER' (21) ถูก truncate
--   แก้แล้วใน delta 2026-09-07-rfa-consent-reasons-code-width.sql (VARCHAR(50))
--
-- เป้าหมาย:
...
ADD COLUMN IF NOT EXISTS `impersonated` TINYINT(1) NOT NULL DEFAULT 0 COMMENT 'ADR-049: 1 = admin ทำ action แทน handler ดั้งเดิม (Superadmin/Org Admin impersonation)',
  ADD COLUMN IF NOT EXISTS `on_behalf_of_user_id` INT NULL COMMENT 'ADR-049: FK → users.id ของ handler ดั้งเดิม (เจ้าของเดิม) — NULL ถ้าไม่ใช่ impersonation',
  ADD COLUMN IF NOT EXISTS `on_behalf_of_user_uuid` VARCHAR(36) NULL COMMENT 'ADR-049: UUID ของ handler ดั้งเดิม สำหรับ API response (ADR-019) — NULL ถ้าไม่ใช่ impersonation';

-- Index สำหรับ query impersonation history ตาม user ดั้งเดิม
CREATE INDEX IF NOT EXISTS `idx_wf_hist_on_behalf_of_user_id` ON `workflow_histories` (`on_behalf_of_user_id`);
...
INSERT IGNORE INTO `rfa_approve_codes` (
    `approve_code`,
    `approve_name`,
    `description`,
    `sort_order`,
...
VALUES (
    '1',
    'Approved',
    'ADR-049: Owner อนุมัติ — transition APPROVE → APPROVED',
    10,
    1
  ),
  (
    ...
    'ADR-049: Owner อนุมัติพร้อมข้อสังเกต — transition APPROVE_WITH_COMMENTS → APPROVED_WITH_COMMENTS',
    20,
    1
  ),
  (
    '3',
    'Revise and Resubmit',
    'ADR-049: สั่งแก้ไข — transition RESUBMIT → REVISE_REQUIRED (CONSULTANT) หรือ → CONSULTANT_REVIEW (OWNER)',
    30,
    1
  ),
  (
    '4',
    'Rejected',
    'ADR-049: ปฏิเสธ — transition REJECT → REJECTED',
    40,
    1
  );
...
-- ============================================================
INSERT IGNORE INTO `rfa_consent_reasons` (
    `public_id`,
    `code`,
    `description`,
    `sort_order`,
