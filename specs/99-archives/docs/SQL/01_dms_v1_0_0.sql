...
CREATE TABLE organizations (
  ...
  organization_code   VARCHAR(20) NOT NULL COMMENT 'รหัสองค์กร',
...
CREATE TABLE projects (
  ...
  contractor_organization_id  INT NULL COMMENT 'รหัสองค์กรผู้รับเหมา (ถ้ามี)',
...
CREATE TABLE users (
  ...
  organization_id          INT NULL COMMENT 'รหัสองค์กร (FK -> organizations)', -- link to organizations for ADMIN scope
  is_active       TINYINT(1) NOT NULL DEFAULT 1 COMMENT 'สถานะการใช้งาน',
...
CREATE TABLE circulations (
  ...
  organization_id         INT NOT NULL COMMENT 'ID ขององค์กรที่เป็นเจ้าของใบเวียนนี้', -- องค์กรที่เป็นเจ้าของใบเวียนนี้
...
CREATE TABLE rfa_revisions (
  ...
  rfa_status_codes_id   INT NOT NULL COMMENT 'รรหัสสถานะเอกสาร (FK → rfa_status_codes.id)',
  rfa_approve_codes_id  INT NULL COMMENT 'รหัสการอนุมัติเอกสาร (FK → rfa_approve_codes.id)',
 ...
 -- Validation constraints
  ...
  CONSTRAINT chk_due_after_received
    CHECK (due_date IS NULL OR received_date IS NULL OR due_date >= received_date),
  CONSTRAINT chk_approved_after_received
    CHECK (approved_date IS NULL OR received_date IS NULL OR approved_date >= DATE(received_date)),
  CONSTRAINT chk_approved_after_due
    CHECK (approved_date IS NULL OR due_date IS NULL OR approved_date >= DATE(due_date)),
  CONSTRAINT chk_issued_after_created
    CHECK (issued_date IS NULL OR created_at IS NULL OR issued_date >= DATE(created_at)),
  CONSTRAINT chk_received_after_created
    CHECK (received_date IS NULL OR created_at IS NULL OR received_date >= created_at),

  -- Foreign keys
 CONSTRAINT fk_rr_correspondence FOREIGN KEY (correspondence_id)
    REFERENCES correspondences(id) ON DELETE CASCADE,
  CONSTRAINT fk_rr_rfa FOREIGN KEY (rfa_id)
    REFERENCES rfas(id) ON DELETE CASCADE,
  CONSTRAINT fk_rr_status FOREIGN KEY (rfa_status_codes_id)
    REFERENCES rfa_status_codes(id),
  CONSTRAINT fk_rr_approve FOREIGN KEY (rfa_approve_codes_id)
    REFERENCES rfa_approve_codes(id) ON DELETE SET NULL,
  CONSTRAINT fk_rr_created_by FOREIGN KEY (created_by)
    REFERENCES users(user_id) ON UPDATE CASCADE ON DELETE SET NULL,
  CONSTRAINT fk_rr_updated_by FOREIGN KEY (updated_by)
    REFERENCES users(user_id) ON UPDATE CASCADE ON DELETE SET NULL
...
CREATE TABLE correspondence_revisions (
  ...
  -- สถานะเฉพาะของ Revision นี้
  is_current                 BOOLEAN NOT NULL DEFAULT FALSE COMMENT 'ระบุว่าเป็น Revision ปัจจุบัน (1=ใช่,0=ไม่ใช่)',
  correspondence_statuss_id   INT NOT NULL COMMENT 'สถานะหนังสือ',
  ...
  CONSTRAINT chk_submitted_after_created CHECK (submitted_date IS NULL OR created_at IS NULL
    OR submitted_date >= DATE(created_at)),
  CONSTRAINT chk_received_after_created CHECK (received_date IS NULL OR created_at IS NULL OR
...
CREATE TABLE circulation_templates (
  id              INT NOT NULL AUTO_INCREMENT CHARACTERPRIMARY KEY COMMENT 'ID ของแม่แบบ',
  template_name   VARCHAR(255) NOT NULL COMMENT 'ชื่อแม่แบบ (เช่น "สำหรับเอกสารการเงิน", "สำหรับ Drawing โครงสร้าง")',
  organization_id INT NOT NULL COMMENT 'ID ขององค์กรที่เป็นเจ้าของแม่แบบนี้ (สำคัญมาก)',
...
CREATE TABLE rfa_workflow_templates (
  id            INT NOT NULL AUTO_INCREMENT COMMENT 'ID ของแม่แบบ',
...
CREATE TABLE global_default_roles (
  ...
  organization_id    INT NOT NULL COMMENT 'ID ขององค์กรที่ถูกกำหนดเป็นค่าเริ่มต้น',
...
SELECT 1, 'OWNER', pos, organization_id
FROM (
  SELECT organization_id, ROW_NUMBER() OVER (ORDER BY organization_id) AS pos
  FROM organizations
...
-- DESIGNER
INSERT INTO global_default_roles (id, role, position, organization_id)
SELECT 1, 'DESIGNER', 1, organization_id
FROM organizations
...
-- CONSULTANT
INSERT INTO global_default_roles (id, role, position, organization_id)
SELECT 1, 'CONSULTANT', 1, organization_id
FROM organizations
...
CREATE TABLE shop_drawing_revision_contract_refs (
  ...
  CONSTRAINT fk_sdrcr_contract_drawing FOREIGN KEY (contract_drawing_id) REFERENCES contract_drawings(condwg_id) ON DELETE CASCADE ON UPDATE CASCADE
...
CREATE TABLE transmittal_items (
  ...
  technical_rfarev_id  INT NOT NULL COMMENT 'ID ของเอกสารทางเทคนิคที่แนบไป',
  ...
  UNIQUE KEY ux_transmittal_item (transmittal_id, technical_rfarev_id),
  
  -- Foreign Keys
  CONSTRAINT fk_ti_transmittal FOREIGN KEY (transmittal_id) REFERENCES transmittals(correspondence_id) ON DELETE CASCADE,
  CONSTRAINT fk_ti_technical_doc FOREIGN KEY (technical_rfarev_id) REFERENCES rfa_revisions(id) ON DELETE CASCADE
...
CREATE TABLE circulation_assignees (
  ...
  circulations_id              INT NOT NULL COMMENT 'ID ของใบเวียน (จากตาราง circulations)',
  ...
  UNIQUE KEY ux_cir_assignee (cir_id, user_id, assignee_type),
  CONSTRAINT fk_ca_assignee_circulation FOREIGN KEY (circulations_id) REFERENCES circulations(id) ON DELETE CASCADE,
...
CREATE TABLE circulation_recipients (
  ...
  -- Foreign Keys
  CONSTRAINT fk_cir_recipients_circulation FOREIGN KEY (circulations_id) REFERENCES circulations(id) ON DELETE CASCADE,
...
CREATE TABLE rfa_items (
  ...
  shop_drawing_revision_id      INT NOT NULL INT NOT NULL UNIQUE COMMENT 'ID ของ shop drawing revison', -- one rfa revision appears in at most one RFA
...
CREATE TABLE rfa_workflow_template_steps (
  ...
  organization_id INT NOT NULL COMMENT 'ID องค์กรที่ต้องพิจารณาในขั้นตอนนี้',
  ...
  CONSTRAINT fk_rwts_template FOREIGN KEY (template_id) REFERENCES rfa_workflow_templates(template_id) ON DELETE CASCADE,
...
CREATE TABLE rfa_workflows (
    ...
    sequence           INT NOT NULL COMMENT 'ลำดับขั้นตอน',
    from_organization_id        INT NOT NULL COMMENT 'ID ขององค์กรผู้ส่ง',
    to_organization_id          INT NOT NULL COMMENT 'ID ขององค์กรผู้รับ',
    ...
    -- Foreign Keys
    CONSTRAINT fk_rfw_correspondence FOREIGN KEY (correspondence_id) REFERENCES correspondences(id) ON DELETE CASCADE,
...
CREATE TABLE correspondence_routing_template_steps (
  ...
  template_id   INT NOT NULL COMMENT 'ID ของแม่แบบ',
  sequence      INT NOT NULL COMMENT 'ลำดับขั้นตอน',
  to_organization_id     INT NOT NULL COMMENT 'ID องค์กรผู้รับในขั้นตอนนี้',
  ...
  CONSTRAINT fk_cwts_template FOREIGN KEY (template_id) REFERENCES correspondence_routing_templates(template_id) ON DELETE CASCADE,
...
CREATE TABLE correspondence_routings (
  ...
  -- Foreign Keys
  CONSTRAINT fk_crs_correspondence FOREIGN KEY (correspondence_id) REFERENCES correspondences(id) ON DELETE CASCADE,
...
CREATE TABLE correspondence_recipients (
  correspondence_id          INT NOT NULL COMMENT 'ID ของเอกสาร (FK -> correspondences)',
  recipient_organization_id  INT NOT NULL COMMENT 'ID ขององค์กรผู้รับ (FK -> organizations)',
  ...
  CONSTRAINT fk_cr_correspondence FOREIGN KEY (correspondence_id) REFERENCES correspondences(id) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT fk_cr_organization FOREIGN KEY (recipient_organization_id) REFERENCES organizations(id) ON DELETE RESTRICT ON UPDATE CASCADE
...
CREATE TABLE correspondence_references (
  ...
  CONSTRAINT fk_ref_src FOREIGN KEY (src_correspondence_id) REFERENCES correspondences(id) ON UPDATE CASCADE ON DELETE CASCADE,
  CONSTRAINT fk_ref_tgt FOREIGN KEY (tgt_correspondence_id) REFERENCES correspondences(id) ON UPDATE CASCADE ON DELETE CASCADE
...
CREATE TABLE contract_parties (
  ...
  project_id  INT NOT NULL COMMENT 'ID ของโครงการ',
  organization_id      INT NOT NULL COMMENT 'ID ขององค์กร',
