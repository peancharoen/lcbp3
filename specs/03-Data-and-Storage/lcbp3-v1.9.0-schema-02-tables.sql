...
CREATE TABLE migration_review_queue (
  ...
  idempotency_key VARCHAR(200) NULL COMMENT 'Idempotency-Key สำหรับป้องกัน queue ซ้ำ (nullable — entity เก่าไม่ได้ใช้)',
  ...
  ai_job_id VARCHAR(150) NULL COMMENT 'BullMQ Job ID สำหรับงานประมวลผล AI (custom jobId เช่น legacy-enrich-<publicId>-<idempotencyKey> ยาวเกิน UUID เปล่า — ADR-047 bugfix 2026-08-23)',
  ai_status ENUM(
    'PENDING',
    'WAITING',
    'RUNNING',
  ...
  temp_attachment_ids JSON NULL COMMENT 'รายการ internal attachment IDs หลายไฟล์ (FR-001, FR-002)',
  compare_status ENUM('COMPARED', 'UNAVAILABLE') NOT NULL DEFAULT 'COMPARED' COMMENT 'สถานะการเปรียบเทียบทะเบียนกับเอกสารจริง (FR-012a)',
  compare_unavailable_reason VARCHAR(500) NULL COMMENT 'เหตุผลภาษาไทยเมื่อ compareStatus = UNAVAILABLE (FR-012b)',
  -- Entity-driven columns (added 2026-08-20, ADR-044 delta sync)
  document_number VARCHAR(100) NULL COMMENT 'เลขที่เอกสารเก่า (จาก OCR/Excel)',
  ...
  remarks TEXT NULL COMMENT 'หมายเหตุจากหน้างาน',
