...
MODIFY COLUMN `uuid` UUID NOT NULL DEFAULT UUID() COMMENT 'UUIDv7 (NestJS @BeforeInsert) สำหรับ runtime; UUIDv1 (DEFAULT UUID() fallback) สำหรับ seed/migration (ADR-019)';
...
-- หมายเหตุ: migration_logs ถูก drop ใน D161 (2026-08-25) — ดู delta 2026-08-25-drop-migration-logs-table.sql
...
ALTER TABLE `ai_intent_definitions`
MODIFY COLUMN `public_id` UUID NOT NULL DEFAULT UUID() COMMENT 'UUIDv7 (NestJS @BeforeInsert) สำหรับ runtime; UUIDv1 (DEFAULT UUID() fallback) สำหรับ seed/migration (ADR-019)';

ALTER TABLE `ai_intent_patterns`
MODIFY COLUMN `public_id` UUID NOT NULL DEFAULT UUID() COMMENT 'UUIDv7 (NestJS @BeforeInsert) สำหรับ runtime; UUIDv1 (DEFAULT UUID() fallback) สำหรับ seed/migration (ADR-019)';
...
ALTER TABLE `response_code_rules`
MODIFY COLUMN `uuid` UUID NOT NULL DEFAULT UUID() COMMENT 'UUIDv7 (NestJS @BeforeInsert) สำหรับ runtime; UUIDv1 (DEFAULT UUID() fallback) สำหรับ seed/migration (ADR-019)';
...
ALTER TABLE `distribution_matrices`
MODIFY COLUMN `uuid` UUID NOT NULL DEFAULT UUID() COMMENT 'UUIDv7 (NestJS @BeforeInsert) สำหรับ runtime; UUIDv1 (DEFAULT UUID() fallback) สำหรับ seed/migration (ADR-019)';
...
MODIFY COLUMN `public_id` UUID NOT NULL COMMENT 'UUIDv7 (NestJS @BeforeInsert, ADR-019) — app layer สร้างเสมอ (ไม่มี DB DEFAULT)';
...
MODIFY COLUMN `id` CHAR(36) NOT NULL COMMENT 'UUIDv4 (TypeORM @PrimaryGeneratedColumn(''uuid'') — ไม่ใช่ ADR-019 publicId) ของ Workflow Definition';

ALTER TABLE `workflow_instances`
MODIFY COLUMN `id` CHAR(36) NOT NULL COMMENT 'UUIDv4 (TypeORM @PrimaryGeneratedColumn(''uuid'') — ไม่ใช่ ADR-019 publicId) ของ Instance';

ALTER TABLE `workflow_histories`
MODIFY COLUMN `id` CHAR(36) NOT NULL COMMENT 'UUIDv4 (TypeORM @PrimaryGeneratedColumn(''uuid'') — ไม่ใช่ ADR-019 publicId)';

ALTER TABLE `document_chunks`
MODIFY COLUMN `id` CHAR(36) NOT NULL COMMENT 'UUIDv4 = Qdrant point ID (TypeORM @PrimaryGeneratedColumn(''uuid'') สำหรับ ai_document_chunks หรือ UUID ที่ Qdrant assign)';

-- =====================================================
-- 5. FK columns ที่อ้างอิง UUID (ระบุที่มา v1/v7)
-- ==========================================================
ALTER TABLE `ai_audit_logs`
MODIFY COLUMN `document_public_id` UUID NULL COMMENT 'Imported document publicId (UUIDv7 หรือ UUIDv1 ตามที่มาของ document) when available';
...
ALTER TABLE `workflow_histories`
MODIFY COLUMN `action_by_user_uuid` VARCHAR(36) NULL COMMENT 'UUID ของ User ผู้ดำเนินการ (คัดลอกจาก users.uuid — อาจเป็น UUIDv7 หรือ UUIDv1 ตามที่มาของ user record) — ใช้ใน API Response แทน INT FK (ADR-019). NULL = System Action หรือ Pre-migration record';
