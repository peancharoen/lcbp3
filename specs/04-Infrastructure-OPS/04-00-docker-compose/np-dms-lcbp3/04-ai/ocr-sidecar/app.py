...
# - 2026-08-26: BGE lazy-load refactor — เปลี่ยน BGE จาก startup-load เป็น lazy-load + keep_alive
#   - แยก BGE-M3 และ Reranker เป็น lazy singletons (โหลดเฉพาะตอนถูกเรียก)
#   - เพิ่ม /bge/load, /bge/unload, /bge/status endpoints สำหรับ coordination กับ Ollama
#   - BGE auto-unload หลัง idle timeout (BGE_KEEP_ALIVE_SECONDS, default 300s)
#   - แก้ปัญหา OCR OOM: BGE กิน 4.8GB GPU ตลอด ทำให้ OCR (ต้องการ 14GB) ไม่พอ VRAM
...
# - 2026-09-03: กู้คืน BGE lazy-load (2026-08-26) ที่หายไปจากการ merge D2/D3 (2026-09-01) — ทั้งสอง feature ควรอยู่ร่วมกันได้ ไม่ขัดแย้งกัน
...
# BGE lazy-load singletons — โหลดเฉพาะตอนถูกเรียก (lazy) พร้อม keep_alive auto-unload
bge_model = None
reranker = None
bge_loaded_at: float | None = None  # timestamp ของการใช้งานล่าสุด (monotonic)
bge_keep_alive_seconds = float(os.getenv("BGE_KEEP_ALIVE_SECONDS", "300"))
bge_lock = asyncio.Lock()  # บังคับ sequential load/unload
...
def _load_bge_model() -> 'BGEM3FlagModel | None':
    """โหลด BGE-M3 model บน CPU RAM (lazy load)"""
    logger.info("Loading BGE-M3 model on CPU RAM...")
    try:
...
def _load_reranker() -> 'FlagReranker | None':
    """โหลด BGE-Reranker-Large model บน CPU RAM (lazy load)"""
    logger.info("Loading BGE-Reranker-Large model on CPU RAM...")
    try:
        rerank = FlagReranker('BAAI/bge-reranker-large', use_fp16=False)
        logger.info("BGE-Reranker-Large model loaded successfully.")
        return rerank
    except Exception as e:
...
def _unload_bge_models():
    """Unload BGE models และคืน memory (CPU RAM + GPU)"""
    global bge_model, reranker, bge_loaded_at
    logger.info("Unloading BGE-M3 and Reranker models...")
    bge_model = None
    reranker = None
    bge_loaded_at = None
    import gc
    ...
    gc.collect()
    logger.info("BGE models unloaded. GPU cache cleared.")


async def _ensure_bge_loaded(need_reranker: bool = False) -> bool:
    """Lazy-load BGE models ถ้ายังไม่โหลด และอัปเดต keep_alive timestamp"""
    global bge_model, reranker, bge_loaded_at
    async with bge_lock:
        if bge_model is None:
            bge_model = await asyncio.to_thread(_load_bge_model)
        if need_reranker and reranker is None:
            reranker = await asyncio.to_thread(_load_reranker)
        if bge_model is not None:
            bge_loaded_at = asyncio.get_event_loop().time()
            return True
        return False


async def _check_bge_keep_alive():
    """Auto-unload BGE ถ้า idle เกิน keep_alive timeout"""
    global bge_loaded_at
    if bge_loaded_at is None or bge_model is None:
        return
    elapsed = asyncio.get_event_loop().time() - bge_loaded_at
    if elapsed > bge_keep_alive_seconds:
        logger.info(f"BGE keep_alive expired ({elapsed:.0f}s > {bge_keep_alive_seconds}s) — auto-unloading")
        async with bge_lock:
            _unload_bge_models()


async def _bge_keep_alive_monitor():
    """Background task: ตรวจ BGE keep_alive ทุก 60 วินาที — auto-unload ถ้า idle"""
    while True:
        await asyncio.sleep(60)
        try:
            await _check_bge_keep_alive()
        except Exception as e:
            logger.warning(f"BGE keep_alive monitor error: {e}")


@asynccontextmanager
async def lifespan(app_instance: FastAPI):
    """T043/T045: Lifespan context manager — จัดการ AsyncClient และ Redis client
    BGE models ไม่โหลดตอน startup แล้ว — lazy load ตอนถูกเรียกผ่าน /embed หรือ /rerank"""
    ...
    try:
        ...
        redis_client = None
    # BGE models: lazy load — ไม่โหลดตอน startup เพื่อคืน GPU ให้ Ollama
    logger.info("BGE models will lazy-load on first /embed or /rerank call")
    # Start keep_alive monitor task
    monitor_task = asyncio.create_task(_bge_keep_alive_monitor())
    yield
    # Cleanup: ปิด AsyncClient, Redis client, และ monitor task
    monitor_task.cancel()
    if ollama_client:
...
@app.post("/bge/load")
async def bge_load():
    """โหลด BGE-M3 และ Reranker models (lazy load + warm up GPU)"""
    success = await _ensure_bge_loaded(need_reranker=True)
    if not success:
        raise HTTPException(status_code=503, detail="Failed to load BGE models")
    return {
        "status": "loaded",
        "bgeLoaded": bge_model is not None,
        "rerankerLoaded": reranker is not None,
    }


@app.post("/bge/unload")
async def bge_unload():
    """Unload BGE models และคืน GPU memory — coordination กับ Ollama ก่อน OCR/AI job"""
    async with bge_lock:
        _unload_bge_models()
    return {
        "status": "unloaded",
...
@app.get("/bge/status")
def bge_status():
    """สถานะ BGE models (loaded/not loaded + keep_alive info)"""
    elapsed = None
    if bge_loaded_at is not None:
        loop = asyncio.get_event_loop()
        elapsed = loop.time() - bge_loaded_at
    return {
        "bgeLoaded": bge_model is not None,
        "rerankerLoaded": reranker is not None,
        "keepAliveSeconds": bge_keep_alive_seconds,
        "idleSeconds": elapsed,
        "autoUnloadIn": max(0, bge_keep_alive_seconds - elapsed) if elapsed is not None else None,
    }
...
async def embed_text(req: EmbedRequest):
    """BGE-M3 embedding generator (Dense + Sparse) พร้อม lazy-load, CPU fallback และ timeout guard"""
    # Lazy load BGE-M3 ถ้ายังไม่โหลด
    loaded = await _ensure_bge_loaded(need_reranker=False)
    if not loaded:
        raise HTTPException(status_code=503, detail="BGE-M3 model failed to load")
...
async def rerank_chunks(req: RerankRequest):
    """BGE-Reranker-Large chunk re-ranker พร้อม lazy-load, CPU fallback และ timeout guard"""
    if not req.chunks:
        return RerankResponse(scores=[], ranked_indices=[], device="cpu")
    # Lazy load BGE-Reranker ถ้ายังไม่โหลด
    loaded = await _ensure_bge_loaded(need_reranker=True)
    if not loaded or reranker is None:
